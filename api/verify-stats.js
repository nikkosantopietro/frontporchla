...
async function diagnose(zoneId) {
  ...
  // ---- DEBUG: why might we get 0 rows? Inspect the actual data shape. -------
  const debug = { serviceKeyPresent: !!process.env.SUPABASE_SERVICE_KEY };

  const { data: zoneAny, error: zoneAnyErr } = await supabase
    .from('listings').select('status').eq('zone_id', zoneId);
  debug.zoneAnyError = zoneAnyErr ? zoneAnyErr.message : null;
  debug.zoneLinkedCount = (zoneAny || []).length;

  const tally = {};
  (zoneAny || []).forEach(r => {
    const k = (r.status === null || r.status === undefined) ? '(null)' : String(r.status);
    tally[k] = (tally[k] || 0) + 1;
  });
  debug.statusTally = tally;

  const { count: totalCount, error: countErr } = await supabase
    .from('listings').select('*', { count: 'exact', head: true });
  debug.totalListings = countErr ? ('error: ' + countErr.message) : totalCount;

  const { data: sample } = await supabase
    .from('listings').select('zone_id, status').limit(8);
  debug.sample = sample || [];
  ...
  const recentSolds = inWindow.filter(r => homeInfo(r).price >= 100000 && homeInfo(r).homeType === 'SINGLE_FAMILY');
  ...
  const activeNonRental = activeRows.filter(r => homeInfo(r).price >= 100000 && homeInfo(r).homeType === 'SINGLE_FAMILY');

  // --- Extra diagnostics: data freshness + duplicate detection ---
  // When were these rows inserted into Supabase (i.e. when scraped)?
  const { data: createdRows, error: createdErr } = await supabase
    .from('listings').select('created_at').eq('zone_id', zoneId);
  if (createdErr) {
    debug.createdRange = 'created_at error: ' + createdErr.message;
  } else {
    const cs = (createdRows || []).map(r => r.created_at).filter(Boolean).sort();
    debug.createdMin = cs[0] || 'n/a';
    debug.createdMax = cs[cs.length - 1] || 'n/a';
  }

  // Are the 'sold in last 12 months' rows actually DISTINCT homes, or duplicates
  // from repeated scrapes? Dedupe by Zillow's zpid.
  const zpidOf = (r) => homeInfo(r).zpid || r.raw_data?.zpid || null;
  const recentZpids = recentSolds.map(zpidOf);
  debug.recentSoldRows = recentSolds.length;
  debug.recentSoldUniqueHomes = new Set(recentZpids.filter(Boolean)).size;
  debug.recentSoldMissingZpid = recentZpids.filter(z => !z).length;

  // Distribution of sale dates by month, to sanity-check the 12-month window.
  const soldByMonth = {};
  recentSolds.forEach(r => {
    const ms = homeInfo(r).dateSold;
    if (ms) {
      const d = new Date(ms);
      const k = d.getUTCFullYear() + '-' + String(d.getUTCMonth() + 1).padStart(2, '0');
      soldByMonth[k] = (soldByMonth[k] || 0) + 1;
    }
  });
  debug.soldByMonth = soldByMonth;
...
// (debug now also carries createdMin/Max, recentSoldUniqueHomes, soldByMonth)
...
<body><div class="wrap">
  ...
  <p class="sub">Zone ${esc(d.zoneId)} · counting SINGLE-FAMILY sales since ${esc(d.cutoffDate)} · ${new Date().toLocaleString()}</p>
  ...
  <div class="card">
    <h2>Data shape (debug)</h2>
    <table>
      ${row('Service key present', d.debug.serviceKeyPresent ? 'yes' : 'NO — reads may be blocked')}
      ${row('Total listings in table', d.debug.totalListings)}
      ${row('Listings linked to this zone_id', d.debug.zoneLinkedCount, 'ignoring status — should be ~327')}
      ${d.debug.zoneAnyError ? row('Zone query error', d.debug.zoneAnyError) : ''}
    </table>
    <p style="margin:14px 0 4px;font-size:13px;color:#6b6058;">Status values found for this zone:</p>
    <pre style="background:#faf6f0;border:1px solid #e8ddd0;border-radius:6px;padding:10px;font-size:12px;overflow:auto;">${esc(JSON.stringify(d.debug.statusTally, null, 2))}</pre>
    <p style="margin:14px 0 4px;font-size:13px;color:#6b6058;">Sample rows (zone_id / status) from the table:</p>
    <pre style="background:#faf6f0;border:1px solid #e8ddd0;border-radius:6px;padding:10px;font-size:12px;overflow:auto;">${esc(JSON.stringify(d.debug.sample, null, 2))}</pre>
  </div>

  <div class="card">
    <h2>Data freshness &amp; duplicates</h2>
    <table>
      ${row('Oldest row scraped (created_at)', d.debug.createdMin || d.debug.createdRange || 'n/a')}
      ${row('Newest row scraped (created_at)', d.debug.createdMax || '')}
      ${row('Sold-in-last-12-mo: rows counted', d.debug.recentSoldRows, 'this is the current "Homes sold"')}
      ${row('Sold-in-last-12-mo: UNIQUE homes', d.debug.recentSoldUniqueHomes, 'distinct zpids — the true count if duplicates exist')}
      ${row('…rows missing a zpid', d.debug.recentSoldMissingZpid)}
    </table>
    <p style="margin:14px 0 4px;font-size:13px;color:#6b6058;">Recent sales by month (sale date):</p>
    <pre style="background:#faf6f0;border:1px solid #e8ddd0;border-radius:6px;padding:10px;font-size:12px;overflow:auto;">${esc(JSON.stringify(d.debug.soldByMonth, null, 2))}</pre>
  </div>
</div></body></html>`;
}
