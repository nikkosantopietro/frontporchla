...
async function diagnose(zoneId) {
  ...
  // How many listings link to this zone at all (ignoring status)?
  const { data: zoneAny, error: zoneAnyErr } = await supabase
    .from('listings').select('status').eq('zone_id', zoneId);
  debug.zoneAnyError = zoneAnyErr ? zoneAnyErr.message : null;
  debug.zoneLinkedCount = (zoneAny || []).length;

  // Tally the distinct status values present for this zone.
  const tally = {};
  (zoneAny || []).forEach(r => {
    const k = (r.status === null || r.status === undefined) ? '(null)' : String(r.status);
    tally[k] = (tally[k] || 0) + 1;
  ...
  // Total rows in the table + a tiny sample of zone_id/status to eyeball.
  const { count: totalCount, error: countErr } = await supabase
    .from('listings').select('*', { count: 'exact', head: true });
  debug.totalListings = countErr ? ('error: ' + countErr.message) : totalCount;
