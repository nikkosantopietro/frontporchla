    ...
    for (const agent of agents) {
      ...
      for (const zoneId of Object.keys(byZone)) {
        ...
        const intro = introHasData
          ? `Here's your ${month} snapshot of <em style="color:#4a6741;">${zone?.name || 'your neighborhood'}</em>. Over the past year, ${marketStats.homesSold} single-family homes have sold here at a median of ${marketStats.medianPrice}, and with ${marketStats.activeListings} on the market today, it's currently a ${introMkt}.`
          : `Here's your ${month} snapshot of <em style="color:#4a6741;">${zone?.name || 'your neighborhood'}</em>.`;
