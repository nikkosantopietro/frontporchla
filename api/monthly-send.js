...
async function getZoneStats(zoneId) {
  ...
  if (soldCount > 0) {
    const monthsOfInventory = activeCount / (soldCount / 12);
    if (monthsOfInventory < 4) stats.marketStatus = "Seller's Market";
    else if (monthsOfInventory > 6) stats.marketStatus = "Buyer's Market";
...
async function getSubscriberValue(sub, zoneId) {
  // Primary: ATTOM AVM for the subscriber's actual address.
  if (sub.address) {
    try {
      const avm = await attom.getAVM(sub.address);
      if (avm && avm.estimate) {
        return { estimate: avm.estimate, low: avm.low, high: avm.high };
      }
    } catch (e) {
      console.error('ATTOM getAVM failed:', e);
    }
  }
  ...
  try {
    ...
    for (const agent of agents) {
      ...
      for (const zoneId of Object.keys(byZone)) {
        ...
        const introMkt = (marketStats.marketStatus || 'Balanced Market').toLowerCase();
        const introHasData = !!(marketStats && marketStats.medianPrice && marketStats.medianPrice !== 'N/A' && String(marketStats.homesSold) !== '0');
        const intro = introHasData
          ? `Here's your ${month} snapshot of ${zone?.name || 'your neighborhood'}. Over the past year, ${marketStats.homesSold} single-family homes have sold here at a median of ${marketStats.medianPrice}, and with ${marketStats.activeListings} on the market today, it's currently a ${introMkt}.`
          : `Here's your ${month} snapshot of ${zone?.name || 'your neighborhood'}.`;
        for (const sub of zoneSubs) {
          try {
            ...
            const templateData = {
              ...
              listingsUrl: 'https://frontporchla.com/api/listings?zone=' + zoneId,
...
async function sendAgentConfirmation(agent, sent, skipped, month, year) {
  const html = `
        ...
        Your monthly neighborhood reports have been delivered. Here's a summary:
        
          Emails sent${sent}
          Skipped (no zone)${skipped}
        
        ${skipped > 0 ? `⚠️ ${skipped} subscriber${skipped > 1 ? 's were' : ' was'} skipped because their address isn't covered by any of your zones. Draw a zone →` : ''}
