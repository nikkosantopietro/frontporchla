...
async function sendHotLeadAlert(sub, agentEmail, reason) {
  ...
  const __ln = notes.length > 0 ? notes[notes.length - 1] : null;
  ...
  const lastNoteDate = __ln && __ln.date ? new Date(__ln.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '';

  const __d = sub.phone ? String(sub.phone).replace(/\D/g, '') : '';
  const __t = __d.length === 11 && __d[0] === '1' ? __d.slice(1) : __d;
  const phoneCell = __t.length === 10 ? '<a href="tel:+1' + __t + '" style="color:#b5652a;text-decoration:none;">(' + __t.slice(0,3) + ')' + __t.slice(3,6) + '-' + __t.slice(6) + '</a>' : (sub.phone || '—');
  const addressCell = sub.address ? '<a href="https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent(sub.address) + '" style="color:#b5652a;text-decoration:none;">' + sub.address + '</a>' : '—';
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;background:#fdfaf7;border:1px solid #e8ddd0;border-radius:12px;overflow:hidden;">
      <div style="background:#b5652a;padding:20px 24px;">
        <p style="margin:0;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#ffffff;">Front Porch LA · Hot Lead Alert</p>
